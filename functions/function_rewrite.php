...
function amp_content($string){
   ...
   $kq = strip_tags($kq,'<p><a><div><figure><figcaption><h2><h3><h4><a><img><span><center><table><tbody><tr><th><td><button><blockquote><strong><ul><li>');
   $dom = new DOMDocument;
   ...
   if(!in_array($node->nodeName, array('src','href','class','id','alt'))) {$node->parentNode->removeAttribute($node->nodeName);}
   }
   $kq = $dom->saveHTML();
   $kq = preg_replace('/^<!DOCTYPE.+?>/', '', str_replace( array('<html>', '</html>', '<body>', '</body>'), array('', '', '', ''), $kq));
 
   $kq = preg_replace('/<img(.+)>/', '<amp-img width="600" height="400" layout="intrinsic" $1 ></amp-img>', $kq);
   return $kq;
 }
