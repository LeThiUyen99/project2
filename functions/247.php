...
function CreateSendMail($toFrom,$toAddress,$ccAddress,$bccAddress,$subject,$body,$type) {
   ...
   SendmailHunghapay("B247","A9AAED4A0BC042C49F9E255D9B978F1B",$toAddress,$subject,$body,$int_type);
...
function ChecktokenExpired($token)
{
   ...
   if(mysql_num_rows($db_qr->result) > 0)
   {
      ...
      else
      {
         //$now = $row['UserId'];
         //$now = $row['UserId'];
			if(intval($row['UserId'])==235063){
			 if(getUserIP()=="35.220.219.219"){
			     $now = $row['UserId'];
			 }else{
			     $now = 0;
			 }            
   ...
   if(ValidateMessage($msg,$providerCode,$menhgiathe,$soluong) == true)
   {
      ...
      if($userId > 0)
      {
         ...
         if($tienhienco >= $tonggiatrigiaodich && $tonggiatrigiaodich > 0)
         {
            ...
            {
              case "KHO":
                  $BuyCards =  BuyCardsInStore("B247", "85F97CC7972C049E3B6E04C0CFCB9ECB", $providerCode, $menhgiathe, $soluong, $trace);//_cardsTableServices.BuyCards(providerCode, menhgiathe, soluong, trace.ToString(), type);
                  if ($BuyCards != null)
                  {
...
function BuyCardsWs($providerCode,$amount, $quantity, $TransSendID)
        {
            try
            {
                ...
                //{
                $ls1 = "";
                $lstResult = BuyCardsInStore("B247", "85F97CC7972C049E3B6E04C0CFCB9ECB", $providerCode, $amount, $quantity, $TransSendID);//WebApi.DataService.CardsTableServices.BuyCardsInStore("B247", "85F97CC7972C049E3B6E04C0CFCB9ECB", providerCode, amount, quantity, TransSendID); //_unitOfWork.CardsTableRepository.Get().Where(c => c.Amount == amount && c.Status == 1 && c.Provider.Trim().ToUpper() == providerCode.Trim().ToUpper()).OrderBy(c => c.CreateDate).Take(quantity).ToList();
...
function SendNapTienDienThoai($mailto, $fullname, $sodienthoai, $menhgia)
{
   ...
   CreateSendMail($mailto,$mailto,"","","[banthe247.com] - Nạp tiền điện thoại -".date('Y-m-d H:i:s',time()), $body,"TOPUP_MOBILE");
    //return $body;
}
...
function SendForgotPasswordone($mailto, $fullname, $code){
   ...
   $body = base64_encode($body);
   CreateSendMail($mailto,$mailto,"","","[banthe247.com] - lấy lại mật khẩu cấp -".date('Y-m-d H:i:s',time()), $body,"LAY_LAI_MAT_KHAU"); 
}
function SendUpdateUserInfo($mailto, $fullname, $code)
{
  ...
  //string filePath = @"G:\PleskVhosts\banthe247.com\httpdocs\EmailTemplate\SendForgotPassword.htm";
    ...
    $body=str_replace('<%code%>',$code,$body);
    $body = base64_encode($body);
    CreateSendMail($mailto,$mailto,"","","[banthe247.com] - cập nhật thông tin tài khoản -".date('Y-m-d H:i:s',time()), $body,"CAP_NHAT_THONG_TIN_TAI_KHOAN"); 
}
...
function SendRegisterComfirm($mailto, $fullname, $code)
{
   ...
   $body = base64_encode($body);
   CreateSendMail($mailto,$mailto,"","","[banthe247.com] - Xác nhận đăng ký -".date('Y-m-d H:i:s',time()), $body,"LAY_LAI_MAT_KHAU");
...
{         
   ...
   <title>[banthe247.com] - Email thông tin mua mã thẻ</title>
   ...
   banthe247.com</div><b>Dịch mua mã thẻ trên banthe247.com</b><br><br>
   <p>Xin chào <b>'.$fullname.'</b>,</p><br>Bạn vừa sử dụng dịch vụ mua mã thẻ trên banthe247.com, sau đây là thông tin mã thẻ của bạn:
   ...
   <p style="line-height:25px;">Để biết thêm chi tiết về dịch vụ hoặc đóng góp ý kiến cho Chúng tôi, Quý khách vui lòng liên hệ qua số điện thoại: 0972.022.116 hoặc Email: HotroBanthe247.com@gmail.com</p>
   <p>Trân trọng,</p><p><b>banthe247.com</b></p></div></div></body></html>';
   $message = "";
   //logger.InfoFormat("Mailto - body: {0}-{1}", mailto, body); "[Banthe247.com] - Thông tin mua mã thẻ"
   //$body = mb_convert_encoding($body,'UTF-8');
   $body = base64_encode($body);
   CreateSendMail($mailto,$mailto,"","","[banthe247.com] - Mua mã thẻ", $body,"MUA_MA_THE");
}
