...
if($action == "login"){
	...
	if($user_id != 0){
		$isAdmin		= 0;
		$db_isadmin	= new db_query("SELECT * FROM useradmin WHERE UserId = " . $user_id);
		$row			= mysql_fetch_array($db_isadmin->result);
		if($row["GroupId"] ==0) $isAdmin = 1;
		//Set SESSION
		$_SESSION["logged"]			= 1;
...
<body bgcolor="#d8f0fa">
	<table id="Table_01" width="361" height="205" align="center" style="margin-top:200px;" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td colspan="3" width="361" height="55" background="" class="title_login" valign="top" style="background-repeat: no-repeat;text-align: center;">Đăng nhập quản trị</td>
		</tr>
		<tr>
			<td rowspan="2"></td>
