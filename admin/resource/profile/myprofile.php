...
if($action == "update_email"){
	$faq_question	= getValue("FullName","str","POST","");
	//Insert to database
	$myform->add("FullName","FullName",0,0,"",1,translate_text("Please_enter_your_email"),0,"");
	//Add table
	$myform->addTable($fs_table);
	//Check Error!
	$errorMsgEmail .= $myform->checkdata();
	if($errorMsgEmail == ""){
		$db_ex = new db_execute($myform->generate_update_SQL("UserId", $admin_id));
...
if($action == "update_password"){
	$Password_old	= getValue("Password_old","str","POST","",1);
	$Password 		= getValue("Password","str","POST","",1);
	//update to database
	$myform->add("Password","Password",4,0,"",1,translate_text("Please_enter_new_password"),0,"");
	//Add table
	$myform->addTable($fs_table);
	//Kiem tra do dai cua mat khau
	$allow_update = 1;
	//check current password
	if (md5($Password_old) != $_SESSION["password"]){
		$allow_update = 0;
...
//Select data
$db_data = new db_query("SELECT * FROM useradmin WHERE UserId = " . $admin_id);
...
function check_form_change_password(){
	...
	if (document.getElementById("Password_con").value==''){
		alert('<?=translate_text("Please_enter_confirm_password")?>');
		return;
	}
	if (document.getElementById("Password_con").value!=document.getElementById("Password").value){
		...
		<td>
			<table border="0" cellpadding="3" cellspacing="2" width="70%" align="center" height="120">
				<form ACTION="<?=$_SERVER['SCRIPT_NAME'] . "?" . @$_SERVER['QUERY_STRING'];?>" METHOD="POST" name="change_email" onsubmit="validateForm(); return false;">
					 ...
					 <tr>
						  <td class="textBold" nowrap="nowrap" align="right"><?="Mã nhân viên"?> :</td>
						  <td><input type="text" name="FullName" id="FullName" value="<?=$row["FullName"]?>" class="form" size="50"></td>
					 </tr>
					 <tr>
						  <td></td>
						  <td>
			...
			<table border="0" cellpadding="3" cellspacing="2" width="70%" align="center">
			 <form ACTION="<?=$_SERVER['SCRIPT_NAME'] . "?" . @$_SERVER['QUERY_STRING'];?>" METHOD="POST" name="change_password">
				  ...
				  <tr>
						...
						<td><input type="password" name="Password_old" id="Password_old" class="form" size="20"></td>
				  </tr>
				  <tr>
						<td class="textBold" nowrap="nowrap" align="right"><?=translate_text("Mật khẩu mới")?> :</td>
						<td><input type="password" name="Password" id="Password" class="form" size="20"></td>
				  </tr>
				  <tr>
						<td class="textBold" nowrap="nowrap" align="right"><?=translate_text("Nhập lại mật khẩu mới")?> :</td>
						<td><input type="password" name="Password_con" id="Password_con" class="form" size="20"></td>
				  </tr>
				  <tr>
