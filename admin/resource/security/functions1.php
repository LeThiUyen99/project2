...
function checkLogin($username, $password){
	...
	$adm_id		= 0;
	$db_check	= new db_query("SELECT UserId 
										 FROM useradmin
...
function get_curent_language(){
	$db_current_language = new db_query("SELECT *
										 FROM useradmin
										 WHERE UserName='" . $_SESSION["userlogin"] . "' AND Password='" . $_SESSION["password"] . "' AND Status=1");
...
function checkaccessmodule($module_id){
	...
	$db_getright = new db_query("SELECT * 
								 FROM useradmin
								 WHERE UserName='" . $userlogin . "' AND Password='" . $password . "' AND Status=1");
	...
	$db_getright = new db_query("SELECT * 
								 ...
								 WHERE UserId = adu_admin_id AND mod_id = adu_admin_module_id AND
								 UserName='" . $userlogin . "' AND Password='" . $password . "' AND Status=1 AND mod_id = " . $module_id);
