...
if($row 	= mysql_fetch_assoc($db_data->result))
{
    if($row['CreateDate'] != '')
    {
        $CreateDate=date('Y-m-d',strtotime($row['CreateDate']));
    }else{
...
$menu 				= 	new menu();
$listAll 			= 	$menu->getAllRecodeNew("faqs","FaqID","",0,"1=1" ,"*","FaqID ASC");//getAllChild("categories_multi","cat_id","cat_parent_id","0",$sql . " AND lang_id = " . $_SESSION["lang_id"] . $sqlcategory,"cat_id,cat_name,cat_order,cat_type,cat_parent_id,cat_has_child","cat_order ASC, cat_name ASC","cat_has_child");
?>
...
<? /*------------------------------------------------------------------------------------------------*/ ?>
<?=template_top(translate_text("Records Edit") . ": " . $row["Title"])?>
<? /*------------------------------------------------------------------------------------------------*/ ?>
...
echo $form->text("Ngày tạo", "CreateDate", "CreateDate", $CreateDate, "Ngày (dd/mm/yyyy)", 0, 100, 18, 10 , " - ", 0, "&nbsp; <i>(Ví dụ: yyyy-mm-dd)</i>");
echo $form->textarea("MetaKeyword", "MetaKeyword", "MetaKeyword", $MetaKeyword, "Keywords", 0, 600, 120, "", "", "");
echo $form->textarea("MetaDescription", "MetaDescription", "MetaDescription", $MetaDescription, "Keywords", 0, 600, 120, "", "", "");
?>
