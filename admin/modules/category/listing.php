...
//var_dump($listAll);die();
$arrayCat = array(0=>translate_text("Category"));
$db_cateogry = new db_query("SELECT CategoryID,CategoryName
									FROM categories
									WHERE Status >= 0 
									ORDER BY CategoryID ASC");
while($row = mysql_fetch_array($db_cateogry->result)){
	$arrayCat[$row["CategoryID"]] = $row["CategoryName"];
}

$list->addSearch(translate_text("Loại danh mục"),"cat_type","array",$array_value);
...
<?=template_top(translate_text("Category listing"),$list->urlsearch())?>
	...
	<table border="1" cellpadding="3" cellspacing="0" class="table" width="100%">
		<tr class="bg"> 
			...
			<td class="bold" align="center"><?=translate_text("Tiêu đề")?></td>
        <!-- <td class="bold" align="center"><?=translate_text("Mô tả")?></td>-->
		...
		<tr <? if($i%2==0) echo ' bgcolor="#FAFAFA"';?>>
			<td>
				<input type="checkbox" name="record_id[]" id="record_<?=$row["CategoryID"]?>_<?=$i?>" value="<?=$row["CategoryID"]?>">
			 </td>
         ...
         <!--<td><?if(!empty($row['Description'])){echo cut_string(strip_tags($row['Description'],'<p>') ,'75','...');}else{echo "nội dung chưa cập nhật";} ?></td>-->
			<td align="center"><a onclick="loadactive(this); return false;" href="active.php?record_id=<?=$row["CategoryID"]?>&type=cat_active&value=<?=abs($row["Status"]-1)?>&url=<?=base64_encode(getURL())?>"><img border="0" src="<?=$fs_imagepath?>check_<?=$row["Status"];?>.gif" title="Active!"/></a></td>											
			<td align="center" width="16"><a class="text" href="edit.php?record_id=<?=$row["CategoryID"]?>&returnurl=<?=base64_encode(getURL())?>"><img src="<?=$fs_imagepath?>edit.png" alt="EDIT" border="0"/></a></td>
			<td align="center"><img src="<?=$fs_imagepath?>delete.png" alt="DELETE" border="0" onclick="if (confirm('Are you sure to delete?')){ window.location.href='delete.php?record_id=<?=$row["CategoryID"]?>&returnurl=<?=base64_encode(getURL())?>'}" style="cursor:pointer"/></td>
		</tr>
		<? } ?>
		</form>
		</table>
