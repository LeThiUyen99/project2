...
$returnurl 		= base64_decode(getValue("returnurl","str","GET",base64_encode("listing_bm.php")));
...
foreach($arr_record as $i=>$record_id){
	...
	$db_check   = new db_query("SELECT *
							 FROM " . $fs_table . "
							 WHERE bmn_id = " . $record_id);
	if($row = mysql_fetch_assoc($db_check->result)){
		//Xóa tin khoa csdl
      if($record_id > 0 && isset($record_id))
      {
         $sql_del = "DELETE FROM ". $fs_table ." WHERE bmn_id = ". $record_id;
