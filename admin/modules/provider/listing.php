...
require_once("inc_security.php");
	...
	unset($class_menu);
	if($listAll != '') foreach($listAll as $key=>$row) $sta_category_id[$row["Id"]] = $row["ProviderCode"];
		/*
	...
	$list->add($field_name, "Tên nhà mạng", "string", 1, 0);
    $list->add('Id', "IdProvider", "string", 1, 0);
    $list->add('ProviderCode', "Code", "string", 1, 0);
	$list->add("ProviderCode2","Code2","number",1,0, 'width="60"');
	$list->add("Status", "Show", "checkbox", 1, 0);
