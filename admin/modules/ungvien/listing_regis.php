...
$list->add("use_create_time","Ngày đăng ký","string",0,0,1);
...
$total		= new db_count("SELECT count(*) AS count 
									 FROM " . $fs_table . "
									 WHERE (use_email <> '' OR use_phone <> NULL) " . $sql);
										 
$total_row = $total->total;							 
$db_listing = new db_query("SELECT * 
									 FROM " . $fs_table . "
									 WHERE (use_email <> '' OR use_phone <> NULL) " . $sql . "
									 ORDER BY " . $list->sqlSort() . " use_create_time DESC " . 
...
$full_path = 'data_ungvien.xlsx';//duong dan file
$objWriter->save($full_path);
header("Content-Type: application/octet-stream");
header("Content-Disposition: attachment; filename=data_ungvien.xlsx");
readfile("https://timviec365.vn/admin/modules/ungvien/data_ungvien.xlsx");
...
<div id="listing">
   ...
   while($row = mysql_fetch_assoc($db_listing->result)){
      ...
      <?=$list->start_tr($i, $row[$id_field]);?>
         ...
         <td align="center"><?=date("d/m/Y",$row['use_create_time'])?></td>    
