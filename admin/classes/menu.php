...
class menu
{
	...
	*/
	function getAllChild($table_name,$id_field,$parent_id_field,$parent_id=0,$where_clause="1",$field_list,$order_clause,$has_child_field=0,$update=1,$level=0,$callback=0)
	{
        ...
        $query="SELECT " . $id_field . "," . $parent_id_field . ", " . $field_list .
										" FROM " . $table_name .
										" WHERE " . $where_clause . 
										" ORDER BY " . $parent_id_field . " ASC," . $order_clause;
                                        
		$db_category = new db_query($query);										

		//gán toàn b? n?i dung t? b?n select ra vào m?ng								
		...
		//g?i hàm x? lý array s?p x?p theo dúng v? trí c?a t?ng record tr? v? giá tr? 0 ho?c 1 d? update hay không
	...
	*/
	...
	//get all parent city
	function getAllPrentCity($id, $type = 0, $raquote = "&raquo;", $class="parentlink"){
		$count_var 		= 0;
		$parent_list 	= '';
		$finish			= false;
		$current_id 	= $id;
		global $path_root;
		while (!$finish){
			$db_getparent = new db_query ("SELECT cou_id,cou_parent_id,cou_name
													 FROM countries " .
													" WHERE cou_id = " . intval($current_id));
			if ($row=mysql_fetch_assoc($db_getparent->result)){
				$parent_list 	= ' <span class="fl"><i class="icons navicon"></i><a class="' . $class . '" href="' . ($type == 2 ? url_city_location($row) : url_city($row, 0, $type)) . '">' . $row["cou_name"] . '</a></span> ' . $parent_list;
		...
		return '<a class="' . $class . ' fl" href="' . $path_root . '">' . translate("Trang chủ") . '</a> ' . $parent_list;
	}
	...
	function getCatcha($id,$level=1){
		...
		if(isset($arrayLevel[count($arrayLevel)-$level]))
			$id = $arrayLevel[count($arrayLevel)-$level];
