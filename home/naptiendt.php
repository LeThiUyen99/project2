...
<head>
   ...
   <title>Nạp tiền điện thoại online giá rẻ</title>
   <meta name="description" content='Nạp tiền điện thoại online chiết khấu cao, nhanh chóng nhất, đăng ký miễn phí tài khoản nạp tiền điện thoại Viettel, Vina, Mobifone nhanh giá rẻ ngay!' />
   <meta name="keywords" content='mua thẻ cào, mua thẻ điện thoại, mua mã thẻ, mua card, thẻ điện thoại' />
   ...
   <link rel="canonical" href='https://banthe247.com/tien-ich/nap-tien-dien-thoai' />
  ...
  <div class="container">
    <div class="row">
      <div class="divcontent1 divmuathe">
        <h1 class="section-title bg-lable2" style="font-size: 22px;text-transform: uppercase;"><span>Nạp tiền điện thoại online</span></h1>
        <div class="form-content bg1 col-md-4"  style="border : none">
         <form name="buycard" action="#" method="post">
           <div class="form-group">
            <table rel="" width="100%">
                                                    <tr>
                                                        <td>
                                                            <input class="form-control" type="text" maxlength="40" autocomplete="off" id="ctrlsoluongthedt" name="ctrlsoluongthedt" value="" placeholder="Số điện thoại" title="Số điện thoại" />
                                                        </td>
                                                    </tr>
                                                </table>
          </div>
                                                        ...
                                                        <tr>
                                                            <td id="tbl_CardAmount">
                                                                <select id="optCardAmount" class="form-control">
                                                                    <option value="0">Chọn mệnh giá thẻ nạp</option>
                                                        ...
                                                        <td>
                                                            <div class="boxdrpmega">
                                                                <select id="thedtPaymentMethod" class="form-control" tabindex="5">
                                                                    <option selected="selected" value="">Phương thức thanh toán</option>
                                                                    <option value="BANTHE24H">Ví Banthe24h</option>
         ...
         <div class="form-group divbt" style="bottom: inherit !important;">
                                                ...
                                                <button type="button" class="btn btn-spec1 btn-md btn-block" id="ctrlmuathedtbtn" style="width:100%;">Nạp tiền ngay</button>
      ...
      <div class="col-md-8 bg1 form-content" style="border:none"> 
        <div id="thongtingiaodich">
                                                ...
                                                <div>
                                                    <label class="control-label bold">* Nạp tiền điện thoại trả trước, trả sau các nhà mạng: <pre style="color:#ff0000;">Viettel, Vinaphone, Mobifone</pre></label>
                                                    <label class="control-label bold">* Thanh toán qua tài khoản ví Banthe24h: <pre style="color:#ff0000;">Nhanh, tiện lợi và miễn phí giao dịch</pre></label>
                                                    <label class="control-label bold">* Đăng ký tài khoản: <pre style="color:#ff0000;">Miễn phí</pre></label>
                                                </div>
                                            </div>
                                            <div id="thedtpreviewmodal" style="display: none;" tabindex="-1" role="dialog" aria-labelledby="myModalLabeltopup" aria-hidden="true">
                                                <div class="">
                                                    <div class="">
                                                        <div class="">
                                                            <h4 class="modal-title" id="myModalLabeltopup"><i class="fa fa-caret-square-o-right"></i>Thông tin giao dịch</h4>
                                                        </div>
                                                        <form class="form-horizontal" role="form">
                                                            <div class="panel-body">
                                                                <div class="row form-horizontal">
                                                                    ...
                                                                    <div class="col-sm-12">
                                                                        <label class="control-label col-sm-4">Thuê bao:</label>
                                                                        <div class="col-sm-7">
    ...
    <? include("../includes/home/inc_linktuychon3.php") ?>
     </div> </div>
  </div>
</div>
    ...
    $(function () {
        ...
        $('#ctrlmuathedtbtn').on("click", function () {
            if (validatethedt()) {
                ...
                $("#lblsotien").text(sumPrice.toLocaleString('en-US', { minimumFractionDigits: 0 }) + " đ, giảm " + (sumAmount-sumPrice).toLocaleString('en-US', { minimumFractionDigits: 0 }) + " đ");
                ...
                if ($('#thedtPaymentMethod').val() == 'BANTHE24H') {
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.Success == false) {
                                ...
                                $("#myModalLabeltopup").html("<b style='color:#ff0000;'>Bạn vui lòng đăng nhập hệ thống để giao dịch</b>");
                    ...
                    $("#myModalLabeltopup").html("<b style='color:#ff0000'>Thanh toán ngay</b>");
            ...
            if ($('#thedtPaymentMethod').val() == 'BANTHE24H') {
                ...
                var _amount = $('#optCardAmount').val();
                    ...
                    $.ajax({
                        ...
                        type: 'POST',
                        data: { sodienthoai: $('#ctrlsoluongthedt').val(), amount: _amount, email: "", telcotype: $('#ProviderIdDT').val() },
                        dataType: 'json',
                        beforeSend: function () {
                            $("#boxLoading").show();
                        },
                ...
                thanhtoanbank($('#thedtPaymentMethod').val(), $('#ctrlsoluongthedt').val(), $('#optCardAmount').val(), $('#ctrlmuathedtemail').val(), $('#ProviderIdDT').val());
            }
        });
        function thanhtoanbank(bankCode, sodienthoai, amount, email, telcotype) {
            $.ajax({
                    ...
                    if (obj.Success == true) {
                        if (obj.Signature == true) {
                            if (obj.data != "") {
                                ...
                                $("#notifyresult").addClass('alert-warning');
                                $('#notifyresult i').addClass('fa-times-circle');
                                $('#notifyresult strong').text('Kết nối với ngân hàng bị gián đoạn, bạn vui lòng chọn ngân hàng khác! ');
                                $('#tblcardinfo').hide();
                            }
                        } else {
                            $('#tblcardinfo').hide();
                            ...
                            $("#notifyresult").addClass('alert-warning');
                            $('#notifyresult i').addClass('fa-times-circle');
                            $('#notifyresult strong').text('Lỗi giao dịch: Hệ thống đang quá tải');
                            if (obj.data) {
