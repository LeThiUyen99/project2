            ...
            $.ajax({
                ...
                success: function (obj) {
                    if (obj.Success == true && obj.status == 1) {
                        ...
                        $($('#old_possword')).tooltip('hide').attr('title', 'Mật khẩu hiện tại không đúng!').tooltip('fixTitle').addClass('errorClass');
                        $('#old_possword').focus();
                    } else {
                        alert('Lỗi');
                    }
                  ...
                  success: function (reponse) {
                      ...
                      if (reponse.Success == true) {
                          var $msgModal = $('#modalinfo').modal({
                             ...
                             .find('.modal-body p').text("Thư khởi tạo mật khẩu đã được gửi thành công. Vui lòng kiểm tra hộp thư đến và thư rác để nhận được hướng dẫn tiếp theo.").end();
                      }
                      else {
                          var $msgModal = $('#modalinfo').modal({
                              backdrop: 'static',
    ...
    self.validateregister = function () {
        ...
        if ($.trim(email) != '') {
            if (!Common.IsValidEmail(email)) {
                $($('#ctrlemailtxt')).tooltip('hide').attr('data-original-title', 'Email không hợp lệ').tooltip('fixTitle').addClass('errorClass');
                flag = false;
            } else {
                $('#ctrlemailtxt').data("title", "").removeClass("errorClass").tooltip("destroy");
            }
...
function BuyCard(){
    ...
    self.loadAllProvider = function () {
        var strHTML = "<td><p><input id='buyCardProviderSelect_Viettel (0%)' name='buyCardProviderSelect' rel='buyCardProvider' value='1' txt='Viettel (0%)' type='radio' class='left'>&nbsp;<label class='left mL5' for='buyCardProviderSelect_Viettel (0%)'>Viettel (0%)</label><span class='separator'></span></p><p><input id='buyCardProviderSelect_Mobifone (0%)' name='buyCardProviderSelect' rel='buyCardProvider' value='2' txt='Mobifone (0%)' type='radio' class='left'>&nbsp;<label class='left mL5' for='buyCardProviderSelect_Mobifone (0%)'>Mobifone (0%)</label><span class='separator'></span></p><p><input id='buyCardProviderSelect_Vinaphone (0%)' name='buyCardProviderSelect' rel='buyCardProvider' value='3' txt='Vinaphone (0%)' type='radio' class='left'>&nbsp;<label class='left mL5' for='buyCardProviderSelect_Vinaphone (0%)'>Vinaphone (0%)</label><span class='separator'></span></p><p><input id='buyCardProviderSelect_Vietnamobile(0%)' name='buyCardProviderSelect' rel='buyCardProvider' value='7' txt='Vietnamobile(0%)' type='radio' class='left'>&nbsp;<label class='left mL5' for='buyCardProviderSelect_Vietnamobile(0%)'>Vietnamobile(0%)</label><span class='separator'></span></p></td>";
...
function naptiendt(){ //nap tiền vào tài khoản qua thẻ điện thoại
    var self = this;
                ...
                if (isVisible) { 
                    if (self.validatelogintopup()) {
                        self.Logintopup($('#ctrlemailtopuptxt_log').val(), $('#ctrlpasstopuptxt_log').val(), function () {//login calback
                            self.topupnotsigin($('#ctrlemailtopuptxt_log').val(), $('#ctrlpasstopuptxt_log').val(), $('#ctrlSoDienThoai').attr('value'), $('#topupAmount').attr('value'), $('#ctrlnapdienthoaiemailaddress').val(),$('#topuptype').attr('value') )
                        });
                    }
                } else {//case has login                
...
//                        }
    ...
    self.validatenaptiendienthoai = function () {
        ...
        if ($.trim(paymethod) == '') {
            $($('#topupPaymentMethod')).tooltip('hide').attr('title', 'Chọn phương thức thanh toán').tooltip('fixTitle').addClass('errorClass');
            flag = false;
        } else {
...
function muathegame() {
    ...
    $('#ctrthanhtoangamebtn').on("click", function () {
        ...
        if ($('#buyCardGamePaymentMethod').attr('value') == 'banthe247') { //case select paycard
            ...
            self.paymentbuycardusesbank($('#buyCardGamePaymentMethod').attr('value'), $('#buyCardGameProvider').attr('value'), $('#ctrlemailaddressgame').val(), $('#buyCardGameAmount').attr('value'), $('#ctrlsoluongthegame').val())
        }
    ...
    self.validatebuycardgame = function () {
        ...
        if ($.trim(paymethod) == '') {
            $($('#buyCardGamePaymentMethod')).tooltip('hide').attr('title', 'Chọn phương thức thanh toán').tooltip('fixTitle').addClass('errorClass');
            flag = false;
        } else {
