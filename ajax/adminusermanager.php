...
if(isset($_GET) && $_SERVER['REQUEST_METHOD'] =='GET')
{
    ...
    if(strpos($headers['Referer'],'/admin/modules'))
    {
        ...
        $denNgay = getValue("denNgay","str","GET","");
        $denNgay = trim(replaceMQ($denNgay));
        $tuNgay = getValue("tuNgay","str","GET","");
        $tuNgay = trim(replaceMQ($tuNgay));
        ...
        $trangThai = getValue("trangThai","int","GET",0);
        ...
        $kq = AdminUserManager($tuNgay ,$denNgay,$findKey,$trangThai,$iDisplayStart,$iDisplayLength);
        ...
        "data" => $kq['kq'],
        "tongTaiKhoan"=>number_format($kq['total'])];
