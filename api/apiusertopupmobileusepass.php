...
//var_dump($headers);die();
 if($headers['Host']==$host)
 {
    ...
    $countstring=explode(':',$msg);
    if($matKhauCap2 != '' && count($countstring)>=2){
        $response=APIUserTopupMobileUsePass($token,$msg,$matKhauCap2);
        header("Content-Type:application/json;charset=utf-8");
        echo json_encode($response,JSON_UNESCAPED_UNICODE);
    }else{
        $kq=array("RepCode"=>999,"Message"=>"Bạn phải truyền dữ liệu","data"=>"");
