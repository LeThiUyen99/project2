...
if(isset($_POST)&& $_SERVER['REQUEST_METHOD'] =='POST'){    
    ...
    $emailnhan = getValue("email","str","GET","");
    $emailnhan = trim(replaceMQ($emailnhan));
    $emailnhan = removeHTML($emailnhan);
    $sodienthoai = getValue("sodienthoai","str","GET","");
    ...
    $telcotype = getValue("telcotype","str","GET","");
...
//var_dump($headers);die();
 if($headers['Host']==$host)
 {
    ...
    if($amount > 0 && $sodienthoai !=''){
        $response=apiaddvnpayment($token,$bankCode,$sodienthoai,$amount,$email,$vnp_TmnCode,$vnp_Url,$vnp_HashSecret,$vnp_Returnurl);
        header("Content-Type:application/json;charset=utf-8");
        echo json_encode($response,JSON_UNESCAPED_UNICODE);
    }else{
        $returnData = array("RepCode" => -2, "Message"=> "l?i giaodichj", "Link" => "");
 echo json_encode($returnData,JSON_UNESCAPED_UNICODE);
    }
    
 }
} else{
     $kq=array("RepCode" => 99, "Message"=> "l?i giaodichj", "Link" => "");
